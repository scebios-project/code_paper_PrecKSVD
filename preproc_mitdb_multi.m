function preproc_mitdb_multi(signal_number)
...
signal_data     = data{signal_numbers == signal_number};        % signal sample data  = matrix, channels = columns
...
x = filtfilt(b, a, signal_data);                                % operates on columns = channels
plot([signal_data, x])
...
% the annotations
signal_ind = 1:size(signal_data,1);
...
for seg_num = 1:size(signal_ind_matrix,2)
    ...
    % Do PCA on each channel
    for ichan = 1:size(seg_x, 2)
        win{ichan} = buffer(seg_x(:,ichan), winlen, winlen-1, 'nodelay');  % buffer with overlap len-1
        P = pca(win{ichan}');
        PCAsize = 32;
        Pr{ichan} = P(:,1:PCAsize);

        %plot(Pr*Pr'*win(:,1:10))
        Cwin{ichan} = Pr{ichan}'*win{ichan};
    end

    filename = sprintf('data/preproc/preproc_mitdb%d_multi_seg%d.mat', signal_number, seg_num);
    fprintf('Saving %s\n', filename);
