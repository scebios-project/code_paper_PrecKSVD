function [X, Y, T, Targets_all, outputs_all] = classifGenerateRoc(data_files, dict_files, detclass, SPalg, Dname, pick_best, titlestr)
...
for ifile = 1:numel(data_files)
    ...
    if strcmp(SPalg, 'OMP')
        ...
        recerror = sum((Cwin - D*OMPcoef).^2);

    elseif strcmp(SPalg, 'dOMP')
        %% Classification with double-sized OMP
        % Concatenate data vertically and run normal OMP
        Cwin_double = cell2mat(Cwin');
        OMPcoef = mexOMP(Cwin_double, D, param);
        % Find reconstruction error
        %winRec = Pr*D*OMPcoef;
        %recerror = (sum((win - winRec).^2));
        recerror = sum((Cwin_double - D*OMPcoef).^2);
        
        
    elseif strcmp(SPalg, 'sOMP')
        %% Classification with Simultaneous-OMP
        %OMPcoef = mexOMP(Cwin, D, param);
        Cwin_tensor = reshape(cell2mat(Cwin), size(Cwin{1},1), size(Cwin{1},2), numel(Cwin));
        coef1 = zeros(size(D,2), size(Cwin_tensor,3), size(Cwin_tensor,2));  % channels on 2nd dim, will permute later
        ...
        %winRec = Pr*D*OMPcoef;
        %recerror = (sum((win - winRec).^2));
        recerror = sum(sum((Cwin_tensor - mult3(D,OMPcoef)).^2, 3), 1);  % sum along channels and columns

    elseif strcmp(SPalg, 'OMPsep')
        %% Classification with: OMP each channel with own D, sum errors
        % Concatenate data vertically and run normal OMP
        for ich = 1:numel(Cwin)
            OMPcoef{ich} = mexOMP(Cwin{ich}, D{ich}, param);
            % Find reconstruction error
            recerror_c{ich} = sum((Cwin{ich} - D{ich}*OMPcoef{ich}).^2);        
        end
        recerror = sum(cell2mat(recerror_c'));
    
    elseif strcmp(SPalg, 'OMPconcat')
        %% Classification with: OMP each channel with own D, sum errors
        % Concatenate data vertically and run normal OMP
        for ich = 1:numel(Cwin)
            OMPcoef{ich} = mexOMP(Cwin{ich}, D, param);
            % Find reconstruction error
            recerror_c{ich} = sum((Cwin{ich} - D*OMPcoef{ich}).^2);        
        end
        recerror = sum(cell2mat(recerror_c'));        
