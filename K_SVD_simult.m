function [D, residuals, D_hist, coef_hist] = K_SVD_simult(data, D0, niter, L)
...
for iiter=1:niter
    fprintf('%s --- K-SVD_simult iter %2d',  datestr(now, 'yy-mm-dd HH:MM:SS'), iiter);
    ...
    unused_atoms = find(sum(abs(coef(:,:,1)),2) == 0);      % coef has same sparsity structure on all channels, so we can look at (:,:,1) only
    used_atoms = find(sum(abs(coef(:,:,1)),2) ~= 0);        % coef has same sparsity structure on all channels, so we can look at (:,:,1) only
